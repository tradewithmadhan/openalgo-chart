...
class ElliottImpulseWavePaneRenderer implements IPrimitivePaneRenderer {
    _points: ViewPoint[];
    _options: ElliottImpulseWaveOptions;
    _selected: boolean;

    constructor(points: ViewPoint[], options: ElliottImpulseWaveOptions, selected: boolean) {
        this._points = points;
...
class ElliottImpulseWavePaneView implements IPrimitivePaneView {
    _source: ElliottImpulseWave;
    _points: ViewPoint[] = [];

    constructor(source: ElliottImpulseWave) {
        this._source = source;
    }
    ...
    renderer() {
        return new ElliottImpulseWavePaneRenderer(this._points, this._source._options, this._source._selected);
    }
}

export interface ElliottImpulseWaveOptions {
    lineColor: string;
    width: number;
    textColor: string;
}

const defaultOptions: ElliottImpulseWaveOptions = {
    lineColor: '#2962FF',
    width: 2,
    textColor: '#2962FF',
};

export class ElliottImpulseWave implements ISeriesPrimitive<Time> {
    _chart: IChartApi;
    _series: ISeriesApi<keyof SeriesOptionsMap>;
    _points: LogicalPoint[];
    _paneViews: ElliottImpulseWavePaneView[];
    _options: ElliottImpulseWaveOptions;
    ...
    constructor(
        ...
        points: LogicalPoint[],
        options?: Partial<ElliottImpulseWaveOptions>
    ) {
        this._chart = chart;
        this._series = series;
        this._points = points;
        ...
        this._paneViews = [new ElliottImpulseWavePaneView(this)];
    }
    ...
    public applyOptions(options: Partial<ElliottImpulseWaveOptions>): void {
