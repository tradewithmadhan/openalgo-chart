...
/**
 * Round a price value to the nearest instrument tick (minMove).
 * This ensures prices align with the instrument's precision.
 * @param price - The raw price value
 * @param minMove - The minimum price movement for the instrument (default 0.01)
 * @returns Rounded price value
 */
export function roundToMinMove(price: number, minMove: number = 0.01): number {
    if (minMove <= 1e-14) {
        return price; // Guard against division by zero
    }
    return Math.round(price / minMove) * minMove;
}

/**
 * Calculate a profit target price based on Entry, Stop, and a Risk multiplier.
 * Used for position tools (Long/Short) to auto-calculate targets.
 * @param entryPrice - The entry price
 * @param stopPrice - The stop loss price
 * @param multiplier - The risk multiplier (e.g., 3 for 3R target)
 * @param minMove - The minimum price movement for rounding
 * @returns The calculated target price
 */
export function calculateRiskRewardTarget(
    entryPrice: number,
    stopPrice: number,
    multiplier: number = 3,
    minMove: number = 0.01
): number {
    const roundedEntry = roundToMinMove(entryPrice, minMove);
    const roundedStop = roundToMinMove(stopPrice, minMove);
    const riskDistance = Math.abs(roundedEntry - roundedStop);
    const rewardDistance = riskDistance * multiplier;

    const isLong = roundedEntry > roundedStop;
    let targetPrice: number;

    if (isLong) {
        targetPrice = roundedEntry + rewardDistance;
        // Enforce minimum distance (1 tick above entry)
        targetPrice = Math.max(targetPrice, roundedEntry + minMove);
    } else {
        targetPrice = roundedEntry - rewardDistance;
        // Enforce minimum distance (1 tick below entry)
        targetPrice = Math.min(targetPrice, roundedEntry - minMove);
    }

    return roundToMinMove(targetPrice, minMove);
}
