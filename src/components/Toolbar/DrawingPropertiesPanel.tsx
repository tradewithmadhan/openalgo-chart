...
export interface DrawingPropertiesPanelProps {
    ...
    onPropertyChange: (property: keyof DrawingDefaults, value: string | number) => void;
    onReset: () => void;
    isVisible: boolean;
...
const DrawingPropertiesPanel: React.FC<DrawingPropertiesPanelProps> = ({
    ...
    const handleWidthChange = useCallback((e: ChangeEvent<HTMLInputElement>): void => {
        onPropertyChange('width', parseInt(e.target.value, 10));
    }, [onPropertyChange]);

    const handleOpacityChange = useCallback((e: ChangeEvent<HTMLInputElement>): void => {
        onPropertyChange('globalAlpha', parseFloat(e.target.value));
    }, [onPropertyChange]);

    const handleStyleChange = useCallback((style: number): void => {
