...
type FieldType = 'number' | 'select' | 'color' | 'boolean';
...
const ColorPicker: FC<ColorPickerProps> = ({ value, onChange, theme }) => {
    ...
    useEffect(() => {
        ...
        const handleClickOutside = (e: MouseEvent): void => {
            if (!((e.target as Element).closest(`.${styles.colorPopover}`)) &&
                swatchRef.current &&
    ...
    const handleOpacityChange = (e: MouseEvent): void => {
    ...
    const handleMouseDown = (e: ReactMouseEvent<HTMLDivElement>): void => {
        handleOpacityChange(e.nativeEvent);
        const handleMouseMove = (movedEvent: MouseEvent): void => handleOpacityChange(movedEvent);
 ...
 */
const IndicatorSettingsDialog: FC<IndicatorSettingsDialogProps> = ({
    isOpen,
    onClose,
    indicatorType,
    settings,
                ...
                return (
                    <div key={field.key} className={styles.fieldRow}>
                        ...
                        <input
                            ...
                            max={field.max}
                            step={field.step || 1}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange(field.key, Number(e.target.value))}
                        />
                    </div>
                );
                    ...
                    <div key={field.key} className={styles.fieldRow}>
                        ...
                        <label className={styles.toggleSwitch}>
                            <input
                                ...
                                onChange={(e: ChangeEvent<HTMLInputElement>) => handleChange(field.key, e.target.checked)}
                            />
