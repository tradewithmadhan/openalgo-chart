 ...
 */
const getMarketStatus = (): MarketState => {
    const now = new Date();
...
export const usePositionTracker = (
    symbols: SymbolInfo[] | null | undefined,
    ...
    const openPricesRef = useRef<Map<string, number>>(new Map()); // Cache opening prices for session
    const previousRanksRef = useRef<Map<string, number>>(new Map()); // Track previous ranks for animation
    ...
    const hydrateData = useCallback(async () => {
        ...
        const validResults = results
            .filter((r): r is PromiseFulfilledResult<TrackedPosition | null> => r.status === 'fulfilled' && r.value !== null)
            .map(r => r.value as TrackedPosition);
    ...
    const handleTickerUpdate = useCallback((ticker: TickerUpdate) => {
        if (!isMountedRef.current) return;
