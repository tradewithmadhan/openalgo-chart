...
type ScaleMode = typeof SCALE_MODES[keyof typeof SCALE_MODES];
type MergeTarget = 'no_scale' | 'left' | 'right';

export interface PriceScaleContextMenuProps {
    visible: boolean;
    x: number;
    y: number;
    priceScaleId?: string;
    ...
    onScalePriceChartOnlyChange?: (value: boolean) => void;
    onInvertScaleChange?: (value: boolean) => void;
    onScaleModeChange?: (mode: ScaleMode) => void;
    onPlusButtonChange?: (value: boolean) => void;
    onMergeScales?: (target: MergeTarget) => void;
    onOpenSettings?: () => void;
    onClose: () => void;
}
...
const PriceScaleContextMenu: React.FC<PriceScaleContextMenuProps> = ({
    ...
    const handleItemClick = <T,>(callback: ((value: T) => void) | undefined, value: T): void => {
        callback?.(value);
    };

    const handleScaleModeClick = (mode: ScaleMode): void => {
