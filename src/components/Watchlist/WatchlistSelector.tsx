...
export interface WatchlistSelectorProps {
    ...
    onCopy?: (id: string, newName: string) => void;
    onAddSection?: () => void;
...
const WatchlistSelector: React.FC<WatchlistSelectorProps> = ({
    ...
    const handleEditClick = (e: React.MouseEvent, wl: WatchlistData): void => {
        e.stopPropagation();
        setEditingId(wl.id);
        setEditName(wl.name);
    };
    ...
    const handleEditKeyDown = (e: KeyboardEvent<HTMLInputElement>, id: string): void => {
        if (e.key === 'Enter') {
            handleEditSubmit(id);
        } else if (e.key === 'Escape') {
            setEditingId(null);
    ...
    const handleExportClick = (): void => {
        if (onExport) {
            ...
            const csvContent = symbols
                .filter((s): s is SymbolItem => typeof s !== 'string' || !s.startsWith('###'))
                .map(s => {
