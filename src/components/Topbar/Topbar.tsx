...
export interface TopbarProps {
    ...
    onToggleFavorite?: (interval: string) => void;
    onAddCustomInterval?: (value: string, unit: string) => void;
    onRemoveCustomInterval?: (value: string) => void;
    onUndo?: () => void;
    onRedo?: () => void;
    onMenuClick?: () => void;
    theme?: string;
...
const Topbar: React.FC<TopbarProps> = ({
    ...
    const [brokerIntervals, setBrokerIntervals] = useState<Set<string> | null>(null);
    ...
    const getFullTimeframeName = (value: string): string => {
    ...
    useEffect(() => {
        let mounted = true;
        const fetchIntervals = async (): Promise<void> => {
            try {
    ...
    const isIntervalSupported = (value: string): boolean => {
        if (!brokerIntervals) return true;
    ...
    const allTimeframes: Timeframe[] = [...filteredTimeframes, ...customIntervals.map(c => ({ ...c, type: 'Custom' }))];

    // Group timeframes
    const groupedTimeframes: Record<string, Timeframe[]> = {
    ...
    const calculatePosition = (ref: React.RefObject<HTMLDivElement>): DropdownPosition => {
        if (ref.current) {
    ...
    const calculateSnapshotPosition = (ref: React.RefObject<HTMLDivElement>): SnapshotPosition => {
        if (ref.current) {
            const rect = ref.current.getBoundingClientRect();
    ...
    useEffect(() => {
        const handleClickOutside = (event: globalThis.MouseEvent): void => {
            ...
            if (timeframeRef.current && !timeframeRef.current.contains(target) && !target.closest(`.${styles.dropdown}`)) {
                setShowTimeframes(false);
            }
            if (chartTypeRef.current && !chartTypeRef.current.contains(target) && !target.closest(`.${styles.dropdown}`)) {
                setShowChartTypes(false);
            }
            ...
            if (layoutRef.current && !layoutRef.current.contains(target) && !target.closest(`.${styles.dropdown}`)) {
                                                    ...
                                                    <>
                                                        <div className={styles.radioGroup} role="radiogroup" aria-orientation="horizontal">
                                                            {(() => {
                                                                ...
                                                                if (showEffectiveNonFav && effectiveNonFav) {
                                                        ...
                                                        {showCustomInput && (
                                                            <div className={styles.customInputForm}>
                                                                ...
                                                                <div className={styles.customBody}>
                                                                    <input type="number" value={customValue} onChange={(e: ChangeEvent<HTMLInputElement>) => setCustomValue(e.target.value)} className={styles.numInput} min="1" />
                                                                    <select value={customUnit} onChange={(e: ChangeEvent<HTMLSelectElement>) => setCustomUnit(e.target.value)} className={styles.unitSelect}>
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        <div className={styles.itemActions}>
                                                                                            <Star
                                                                                                ...
                                                                                                onClick={(e: MouseEvent) => { e.stopPropagation(); onToggleFavorite?.(tf.value); }}
                                                                                            />
                                                                                            {tf.type === 'Custom' && (
                                                                                                <Trash2
                                                                                                    size={14}
                                                                                                    className={styles.trashIcon}
                                                                                                    onClick={(e: MouseEvent) => { e.stopPropagation(); onRemoveCustomInterval?.(tf.value); }}
                                                                                                />
                                                                                            )}
                                                                                        </div>
                                                                                    </div>
                                                    ...
                                                    <div className={styles.icon}>
                                                        {theme === 'dark' ? (
                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 28" width="28" height="28" fill="currentColor">
                                                                <path d="M14 5a1 1 0 0 1-1-1V2a1 1 0 0 1 2 0v2a1 1 0 0 1-1 1ZM14 27a1 1 0 0 1-1-1v-2a1 1 0 0 1 2 0v2a1 1 0 0 1-1 1ZM27 14a1 1 0 0 1-1 1h-2a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1ZM5 14a1 1 0 0 1-1 1H2a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1ZM14 8a6 6 0 1 0 0 12 6 6 0 0 0 0-12Zm-5 6a5 5 0 1 1 10 0 5 5 0 0 1-10 0Z"></path>
                                                            </svg>
                                                        ) : (
